...
#
# Note: ferry writes progress to stderr; we merge it (2>&1) so PowerShell shows it
# as normal output instead of red error text. Do NOT set $ErrorActionPreference=Stop
# here - PS 5.1 would treat that stderr as a fatal error.
...
# 1. the eval set - a few of your own prompts (written UTF-8 with no BOM)
$evals = @'
[
...
'@
[System.IO.File]::WriteAllText("$PWD\evals.json", $evals)
...
Get-Content ferry-report.md -Encoding utf8
Start-Sleep -Seconds 2

# 4. machine-readable twin for CI
Line "type ferry-report.json | Select -First 12   # same numbers, for CI gates"
Get-Content ferry-report.json -Encoding utf8 | Select-Object -First 12
Write-Host ""
